    ...
    T: Node + Clone,
{
    ...
    fn build_helper(&mut self, curr_node: usize, i: usize, j: usize, values: &[T]) {
        if i == j {
            self.nodes[curr_node].write(values[i].clone());
            return;
        }
        let mid = (i + j) / 2;
        ...
        let (top_nodes, bottom_nodes) = self.nodes.split_at_mut(curr_node + 1);
    ...
    fn update_helper(
        ...
        if i == j {
            self.nodes[curr_node].write(Node::initialize(value));
            return;
        }
        let mid = (i + j) / 2;
        let left_node = 2 * curr_node + 1;
    ...
    fn query_helper(
        ...
        let right_node = 2 * curr_node + 2;
        if left <= i && j <= right {
            return Some(unsafe { self.nodes[curr_node].assume_init_ref() }.clone());
        }
        match (
        ...
        G: Fn(&<T as Node>::Value, <T as Node>::Value) -> <T as Node>::Value,
    {
        ...
        let left_node = 2 * curr_node + 1;
        let right_node = 2 * curr_node + 2;
        let left_value = unsafe { self.nodes[left_node].assume_init_ref() }.value();
    ...
    T: core::fmt::Debug,
{
    pub(crate) fn dbg(&self) {
        self.dbg_helper(0, 0, self.n);
    }

    fn dbg_helper(&self, curr_node: usize, i: usize, j: usize) {
        let mid = (i + j) / 2;
        let left_node = 2 * curr_node + 1;
        let right_node = 2 * curr_node + 2;
        if i == j {
        ...
        self.dbg_helper(left_node, i, mid);
        self.dbg_helper(right_node, mid + 1, j);
        eprintln!("[{i},{j}]: {:?}", unsafe {
            self.nodes[curr_node].assume_init_ref()
        });
    }
}
#[cfg(test)]
